...
type IGlobalFlowTelemetry = kwskfs.service.report.telemetry.IGlobalFlowTelemetry;
type ISellerFlowTelemetry = kwskfs.service.report.telemetry.ISellerFlowTelemetry;
...
export async function main() {
    ...
    const globalTelemetries = await kwskfs.service.report.telemetry.searchGlobalFlow({
    ...
    const sellerTelemetries = await kwskfs.service.report.telemetry.searchSellerFlow({
        measuredFrom: measuredFrom.toDate(),
        measuredThrough: dateNowByUnitTime.toDate()
    })({ telemetry: telemetryRepo });
    ...
    await Promise.all(restaurants.map(async (restaurant) => {
        ...
        await reportNumberOfTransactionsByStatuses(
            restaurant.name.ja, telemetriesBySellerId, measuredFrom.toDate(), dateNowByUnitTime.toDate()); // ステータスごとの取引数
        await reportExpiredRatio(restaurant.name.ja, telemetriesBySellerId, measuredFrom.toDate(), dateNowByUnitTime.toDate()
        );
        await reportTimeLeftUntilEvent(restaurant.name.ja, telemetriesBySellerId, measuredFrom.toDate(), dateNowByUnitTime.toDate());
        await reportTransactionRequiredTimes(
            restaurant.name.ja, telemetriesBySellerId, measuredFrom.toDate(), dateNowByUnitTime.toDate()
        ); // 平均所要時間
...
async function reportNumberOfTrialsOfTasks(telemetries: IGlobalFlowTelemetry[], measuredFrom: Date, measuredThrough: Date) {
    const targetTaskNames = Object.keys(kwskfs.factory.taskName).map((k) => (<any>kwskfs.factory.taskName)[k]);
